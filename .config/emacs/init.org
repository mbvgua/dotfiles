...
- [[#core-setup--performance][Core Setup & Performance]]
  ...
  - [[#dired][Dired]]
  - [[#windows][Windows]]
  - [[#eldoc--eldoc-box][Eldoc & Eldoc-Box]]
  ...
  (use-package window
    ...
    (display-buffer-alist
     '(
       ...
       ;; keeps it always on right side using 35% of the available space:
       ("\\*\\(Backtrace\\|Warnings\\|Compile-Log\\|[Hh]elp\\|[Hh]elpful\\|Messages\\|Bookmark List\\|Ibuffer\\|Occur\\|eldoc.*\\)\\*"
        (display-buffer-in-side-window)
        (window-width . 0.35)
        (side . right)
        (slot . 0))
...
#+begin_src emacs-lisp
  (use-package eldoc
    ...
    ;; Make sure Eldoc prioritizes Flymake errors over standard documentation
...
More examples [[https://github.com/emacs-evil/evil-surround?tab=readme-ov-file#examples][here]]
...
*** Evil Tutor
Getting acquainted with evil mode in emacs. good for comaparing it to the original vim tutor.
...
#+begin_src emacs-lisp
  ...
  (use-package general
    ...
    (start/leader-keys
      ...
      "b x" '(kill-current-buffer :wk "[b]uffer kill")
      ...
      "f d" '(project-find-dir :wk "[f]ind [d]irectory in project")
      ...
      "g d" '(magit-diff-buffer-file :wk "current ma[g]it file [d]iff")
      "g l" '(magit-log-current :wk "ma[g]it [l]og current file")
      ...
      ;; misc
      ;; "," '(emabark-act :wk "open embark actions")   ;; dont know what emabark does
  	"t" '(:ignore t :wk "toggle settings")
  	;; "t t" '(toggle/terminal :wk "toggle terminal") ;; FIXME:make it work
      "t l" '(display-line-numbers-mode :wk "toggle line numbers")
      "t p" '(toggle/tui-enable-transparency :wk "toggle transparency")
      ...
      ;; eglot
      "c" '(:ignore t :wk "eglot & LSP servers")
      "c a" '(eglot-code-actions :wk "[c]ode [a]ctions")
      "c d" '(eldoc-doc-buffer :wk "generate [c]ode [d]ocumentation for buffer")
      "c f" '(apheleia-format-buffer :wk "[c]ode [f]ormat")
      "c r" '(eglot-rename :wk "[c]ode [r]ename")
      "c e" '(consult-flymake :wk "[c]onsult flymake for [e]rrors")
      ))
...
#+begin_src emacs-lisp
  ...
  ;; Goto definition is inbuilt with Evil 'gd' keybing, no need for this now
  ;; (evil-define-key 'normal 'global (kbd "gd") 'xref-find-definitions) ;; evil-collection already provides gd
  (evil-define-key 'normal 'global (kbd "gr") 'xref-find-references) ;; Goto references
  (evil-define-key 'normal 'global (kbd "gi") 'eglot-find-implementation) ;; Goto implementations
  (evil-define-key 'normal 'global (kbd "K") 'eldoc-box-help-at-point) ;; popup for manual
...
This is a bit distracting off-the-bat, made it appear only when i toggle it on/off
#+begin_src emacs-lisp
...
** Minions
A package that cleans up mode-line clutter. Instead of displaying a long, messy list of every active minor mode, it hides them behind a single, customizable menu icon (like =≡= or =+=). Clicking the icon opens a pop-up menu where you can still easily view and toggle your minor modes without sacrificing valuable screen space.

#+begin_src emacs-lisp
  (use-package minions
    :config
    (setq minions-mode-line-lighter " ✦")    ;; change default indicator to sth better
    (add-to-list 'minions-prominent-modes 'flymake-mode)    ;; keep flymake always
    (minions-mode 1))
...
#+begin_src emacs-lisp
  ...
  (mason-setup
    (dolist (pkg '("clangd" "cpptools" "bash-language-server" "basedpyright" "lua-language-server" ;; "pyright"
                   "jdtls" "json-lsp" "yaml-language-server" "typescript-language-server"
                   "docker-compose-language-service"
  				 ;; formatter
                   "black" "stylua" "prettier"))
      (unless (mason-installed-p pkg)
        (ignore-errors (mason-install pkg)))))
#+end_src

** Apheleia
Allows for asynchronous code formatting using specialized tools.

#+begin_src emacs-lisp
  ;;NOTE: uses 'black', 'prettier' and 'gofmt'
  ;; make it format lua files with stylua
...
#+begin_src emacs-lisp
  ;; TODO: make this versioned to avoid breaking changes
  ...
  ;; Map .ts files to typescript-ts-mode. had to do this manually each time
  ;; HACK: maps '.ts' files to 'typescript-ts-mode' instead of doing it manually
  (add-to-list 'auto-mode-alist '("\\.ts\\'" . typescript-ts-mode))

  ;; mode remappings to allow emacs use modes with treesitter support
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    ;; but in langs with a heavy lsp, e.g lua, emacs hangs entirely
    ;; (setq consult-preview-key 'any)
    (setq consult-preview-key "C-.")
    ;; (setq consult-preview-key '("C-n" "C-p"))
...
**** Work with EaT
#+begin_src emacs-lisp :tangle no
;; ─── Emacs Server (start if not already running) ────────────────────────────
(require 'server)
...
(use-package vterm
  ...
  ;; Allow vterm to use full terminal features
  (setq vterm-shell (executable-find "bash"))) ;; or "zsh", "fish"

;; ─── vterm-toggle (pop terminal in/out) ─────────────────────────────────────
(use-package vterm-toggle
  :ensure t
  :config
  (setq vterm-toggle-scope 'project)) ;; terminal is scoped to current project

;; ─── Terminal split functions ────────────────────────────────────────────────
(defun my/vterm-split-right ()
  "Open a new vterm in a right vertical split."
  (interactive)
  (split-window-right)
  (other-window 1)
...
(defun my/vterm-split-below ()
  "Open a new vterm in a bottom horizontal split."
  (interactive)
  (split-window-below)
  (other-window 1)
  (vterm (generate-new-buffer-name "*vterm*")))
...
(defun my/vterm-new ()
  "Open a standalone new vterm buffer."
  (interactive)
  (vterm (generate-new-buffer-name "*vterm*")))

;; ─── Keybindings via Evil leader ─────────────────────────────────────────────
;; Assumes you are using general.el for leader key bindings.
;; If you use evil-leader or which-key differently, adapt accordingly.
...
(use-package general
  ...
  (general-define-key
   ...
   :prefix "SPC"  ;; change to "," if that's your leader

   ;; Terminal bindings under SPC .
   ". ." #'vterm-toggle          ;; toggle terminal in/out
   ". t" #'my/vterm-new          ;; new terminal buffer
   ". r" #'my/vterm-split-right  ;; split right with terminal
   ". b" #'my/vterm-split-below  ;; split below with terminal

   ;; Window navigation (tmux-style pane switching)
   ". h" #'windmove-left
   ". l" #'windmove-right
   ". k" #'windmove-up
   ". j" #'windmove-down
   ...
   ;; Window resizing
   ". H" (lambda () (interactive) (evil-window-decrease-width 5))
   ". L" (lambda () (interactive) (evil-window-increase-width 5))
   ". K" (lambda () (interactive) (evil-window-decrease-height 5))
   ". J" (lambda () (interactive) (evil-window-increase-height 5))

   ;; Close current window (like tmux pane close)
   ". x" #'delete-window
   ". o" #'delete-other-windows)) ;; keep only current pane

;; ─── Make Evil play nicely inside vterm ─────────────────────────────────────
;; vterm needs to capture all keys, so we drop to emacs state inside it
(evil-set-initial-state 'vterm-mode 'emacs)

;; But still let SPC . work inside vterm for split/nav
(with-eval-after-load 'vterm
  (define-key vterm-mode-map (kbd "C-SPC") nil) ;; free up C-SPC if needed
  (general-define-key
   :states 'emacs
   :keymaps 'vterm-mode-map
   :prefix "C-SPC" ;; use C-SPC as leader inside terminal (SPC is consumed by shell)
   "." #'vterm-toggle
   "r" #'my/vterm-split-right
   "b" #'my/vterm-split-below
   "t" #'my/vterm-new
   "x" #'delete-window
   "h" #'windmove-left
   "l" #'windmove-right
